...
syslogng_root_dir = os.environ['top_builddir']

def is_premium():
    version = os.popen('%s/syslog-ng/syslog-ng -V' % syslogng_root_dir, 'r').read()
    if version.find('premium-edition') != -1:
        return True
    return False

def has_module(module):
    avail_mods = os.popen('%s/syslog-ng/syslog-ng -V | grep ^Available-Modules: ' % syslogng_root_dir, 'r').read()
