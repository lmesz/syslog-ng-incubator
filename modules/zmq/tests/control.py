...
def start_syslogng(conf, keep_persist=False, verbose=False):
    ...
    if syslogng_pid == 0:
        ...
        rc = os.execl('%s/syslog-ng/syslog-ng' % top_builddir, '%s/syslog-ng/syslog-ng' % top_builddir, '-f', 'test.conf', '--fd-limit', '1024', '-F', verbose_opt, '-p', 'syslog-ng.pid', '-R', 'syslog-ng.persist', '--no-caps', '--enable-core', '--seed', '--module-path', module_path)
        sys.exit(rc)
    time.sleep(5)
